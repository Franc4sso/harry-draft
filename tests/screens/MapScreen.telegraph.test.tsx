...
describe('MapScreen telegraph', () => {
  ...
  it('does NOT telegraph synergies on a regular battle node (user directive: no synergy tags)', () => {
    const map: RunNode[] = [
    ...
    // No boss name → no telegraph badge at all, and never a synergy chip.
    expect(screen.queryByTestId('telegraph-a0f1n0')).toBeNull()
    expect(document.querySelector('[data-synergy="slytherin3"]')).toBeNull()
  })
