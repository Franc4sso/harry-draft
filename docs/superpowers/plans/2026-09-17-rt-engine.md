...
export function koUnit(state: RtState, target: RtUnit, source?: RtUnit): boolean {
  ...
  if (source) {
    ...
    if (ss.mods.sogliaBonusPerKo) ss.sogliaBonus = Math.min(ss.mods.sogliaBonusPerKo.cap, round1(ss.sogliaBonus + ss.mods.sogliaBonusPerKo.step) )
...
import { createRng } from '@/game/engine/rng'
import { ability, squad } from './fixtures'
...
describe('castSpell — verbi', () => {
  ...
  it('cura → squadraCura in coda, Untore aggiunge Veleno; scudo; carica; protego; rianima; buff', () => {
    ...
    const hp = s.sides[0].hp; castSpell(s, unitAt(s, 'left', 4)!); expect(s.sides[0].hp).toBe(Math.min(s.sides[0].hpMax, hp + 20))   // nessun KO → cura 20
...
export function castSpell(state: RtState, u: RtUnit, opts: { innesco?: boolean } = {}): boolean {
  ...
  // Crescendo: prima del cast, così il primo cast parte da 0
  const c = sp.crescita
  if (c?.kind === 'crescendo') {
    if (c.trigger === 'lancio') u.crescendo += 1
    else if (c.trigger === 'lancioSenzaGelo') u.crescendo = u.lastGeloAt > u.lastCastAt ? 1 : u.crescendo + 1
  }
  u.casts += 1
...
describe('simulateRt', () => {
  ...
  it('Gelo ferma il timer, Lentezza lo dimezza', () => {
    const left = squad({ id: 'g', stats: { ...A, spd: 38 }, spell: { id: 'gl', name: 'Glacius', desc: '', verb: 'status', gelo: 3 } })
  ...
  it('memoriaDelta e reazioni nel risultato; koLeft/koRight', () => {
    ...
    expect(r.reazioni.Miasma).toBeGreaterThan(0); expect(r.memoriaDelta['left:a']).toBe(r.reazioni.Miasma)
