...
**La squadra nemica è generata UNA SOLA VOLTA, a map-build time, e salvata INTERA nel `RunNode`. Il
resolver di combattimento la LEGGE dal nodo invece di rigenerarla.** Una sola fonte di verità → zero
possibilità di divergenza preview-vs-realtà. `DraftedWizard` è dati puri (serializzabile JSON, il save è
già `saveRun(next)` in localStorage), quindi salvarla nel nodo è sicuro. Il nodo diventa più pesante —
accettato come costo per eliminare la classe di bug.
...
- Genera la squadra con `themedEnemyTeam(rng-fork, area, nodeType, budget)` usando il fork RNG canonico.
- Salva nel nodo: `RunNode.enemyTeam?: DraftedWizard[]` (la squadra completa) + `RunNode.preview?: {
  synergyIds: string[]; bossName?: string }` (derivato da `detectSynergies(enemyTeam)`, per il telegrafo).
- Il resolver di combattimento (`resolveCombat`) usa `node.enemyTeam` se presente, invece di chiamare
  `generateEnemyTeam`. (Fallback alla generazione solo se il nodo è legacy senza enemyTeam — retro-compat
  per i save vecchi.)
- Nuovo campo su `RunNode` (types/run.ts): `enemyTeam?`, `preview?`. Serializzati nel save.
- ⚠️ Save size: la squadra include `Spell` objects. Se il save cresce troppo, salvare una forma compatta
  (`{ wizardId, stats, currentHp?, spellId }`) e ri-idratare a load — DECISIONE in plan; per ora la forma
  piena va bene (è solo dati).
...
- **Balance** (`campaignBalanceB`): ⚠️ squadre a tema sono PIÙ FORTI di squadre miste a parità di budget
  (le sinergie aggiungono potenza). Quindi questa feature ALZA la difficoltà oltre il budget previsto →
  campaignBalanceB scenderà. RI-TARATURA ATTESA (non sorpresa): probabilmente abbassare `themeStrength`
  base o compensare con budget/menace per riportare Grifondoro in [0.15, 0.45]. Tarato sui numeri.
...
- **Retro-compat save vecchi**: un nodo senza `enemyTeam` → il resolver rigenera (path legacy). Nuovi run
  usano il nodo. Nessuna rottura dei salvataggi esistenti.
...
1. Tipi: `RunNode.enemyTeam?: DraftedWizard[]`, `RunNode.preview?: { synergyIds; bossName? }`.
2. `themedEnemyTeam` in teamGen.ts (themeStrength + scelta tema pesata + realizzazione) + test.
3. `generateArea`: pre-genera enemyTeam + preview su ogni nodo combat, col fork canonico + test coerenza.
4. `resolveCombat`: legge `node.enemyTeam` (fallback legacy) — una sola fonte.
5. UI: badge sinergie su `MapScreen` dai `preview`.
6. Balance: ri-tara `themeStrength`/budget → campaignBalanceB in banda. Full suite + tsc.
7. Backlog doc.
