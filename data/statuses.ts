...
export const STATUS_DEFS: StatusDef[] = [
  ...
  // Burn ACCUMULATES like veleno: every fire spell funnels into this one statusId (see
  // normalizeSpell) so all fire DoTs merge into a single accumulating status — one flame icon,
  // damage/turn = per-spell tickAmount × stacks (capped at 8). Was stack:'extend' (single flat
  // fire); switched to accumulate so incendio/confringo/crucio/fiendfyre no longer each push a
  // separate un-mergeable inline dot (which showed as N duplicate flame icons).
  // OLD comment kept for context: re-igniting a burning target EXTENDS its duration
  // (2+2+…) rather than adding parallel instances. Damage/turn stays flat at tickDamage.
  { id: 'burn', name: 'Bruciatura', kind: 'dot', family: 'dot', tickDamage: 8, defaultDuration: 2, stack: 'accumulate', maxStacks: 8, priority: 50, removable: true },
